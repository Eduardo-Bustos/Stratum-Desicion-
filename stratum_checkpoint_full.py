...
# Panel data (opcional si está instalado)
try:
...
# ── Rutas ───────────────────────────────────────────────────
BASE       = "/content/drive/MyDrive/stratum" if IN_COLAB else "./stratum"
...
print(f"✔  Rutas creadas  |  timestamp: {timestamp}")
print(f"   BASE      : {BASE}")
...
def resolve_regime_column(df: pd.DataFrame) -> pd.DataFrame:
    """Consolida la columna Regime_economic independientemente del sufijo."""
    if "Regime_economic" in df.columns:
        pass  # ya existe limpia
...
# Forward-fill dentro de cada entidad para evitar contaminación cruzada
entity_col = next(
...
if entity_col:
    ...
    print(f"  → ffill aplicado por entidad: '{entity_col}'")
else:
    final["Regime_economic"] = final.sort_values("date")["Regime_economic"].ffill()
    print("  → ffill aplicado globalmente (sin columna de entidad)")

# Mapear a etiquetas legibles
REGIME_LABELS = {0: "Recesión", 1: "Expansión", 2: "Transición"}
if final["Regime_economic"].nunique() <= len(REGIME_LABELS):
    final["Regime_label"] = final["Regime_economic"].map(REGIME_LABELS).fillna("Desconocido")
else:
    final["Regime_label"] = "Régimen_" + final["Regime_economic"].astype(str)

print(f"\nDistribución de regímenes (filas):")
print(final["Regime_economic"].value_counts(dropna=False).to_string())

# ============================================================
# 4. LIMPIEZA Y VALIDACIÓN DEL DATASET
# ============================================================
print("\n" + "="*60)
print("4. LIMPIEZA Y VALIDACIÓN")
print("="*60)

# Columnas numéricas candidatas a variables del modelo
...
# Reporte de nulos antes de limpiar
null_report = final[SYSTEMIC_VARS + ["Regime_economic"]].isnull().sum()
print(f"\nNulos por columna (antes de limpiar):\n{null_report.to_string()}")

# Eliminar filas donde ALL variables sistémicas sean nulas
before = len(final)
final = final.dropna(subset=SYSTEMIC_VARS, how="all")
after = len(final)
print(f"\nFilas eliminadas (todas las vars sistémicas nulas): {before - after}")
print(f"Dataset final: {final.shape}")

# Detectar duplicados temporales
if entity_col:
    dupes = final.duplicated(subset=[entity_col, "date"]).sum()
else:
    dupes = final.duplicated(subset=["date"]).sum()
print(f"Duplicados (entidad-fecha): {dupes}")
if dupes > 0:
    final = final.drop_duplicates(subset=[entity_col, "date"] if entity_col else ["date"])
    print(f"  → Duplicados eliminados. Dataset: {final.shape}")
...
def _apply(series_fn, col):
    if grp:
        return grp[col].transform(series_fn)
    return final[col].transform(series_fn)

# Variables rezagadas (lag 1 y lag 2)
for var in SYSTEMIC_VARS:
    final[f"{var}_lag1"] = _apply(lambda s: s.shift(1), var)
    final[f"{var}_lag2"] = _apply(lambda s: s.shift(2), var)

# Primeras diferencias
for var in SYSTEMIC_VARS:
    final[f"d_{var}"] = _apply(lambda s: s.diff(), var)
...
print(f"Dummies de régimen creadas: {REGIME_DUMMY_COLS}")

# Indicador de cambio de régimen
final["regime_change"] = (
    final["Regime_economic"] != _apply(lambda s: s.shift(1), "Regime_economic")
).astype(int)
print(f"Cambios de régimen detectados: {final['regime_change'].sum()}")

# Normalización Z-score de variables sistémicas
...
for ax, var in zip(axes, SYSTEMIC_VARS):
    # Sombrear por régimen
    dates = final["date"].values
    regimes = final["Regime_economic"].values
    for regime_val in sorted(final["Regime_economic"].unique()):
        ...
        ax.fill_between(
            ...
            alpha=0.15,
            color=REGIME_COLORS.get(int(regime_val), "#aec7e8"),
            label=f"Régimen {int(regime_val)}"
        )
    # Serie temporal
    if entity_col:
...
plt.tight_layout()
fig_path = f"{FIGURES}/systemic_series_{timestamp}.png"
plt.savefig(fig_path, dpi=150, bbox_inches="tight")
plt.show()
print(f"  ✔ Figura guardada: {fig_path}")

# Matriz de correlación por régimen
for rv in sorted(final["Regime_economic"].dropna().unique()):
    ...
    sns.heatmap(corr, annot=True, fmt=".2f", cmap="RdBu_r",
                center=0, ax=ax_c, linewidths=.5, square=True)
    ax_c.set_title(
        f"Correlaciones — Régimen {int(rv)} ({REGIME_LABELS.get(int(rv), '')})",
        fontsize=12
    )
    plt.tight_layout()
    fig_path_c = f"{FIGURES}/corr_regime{int(rv)}_{timestamp}.png"
    plt.savefig(fig_path_c, dpi=150, bbox_inches="tight")
    plt.show()
    print(f"  ✔ Correlaciones régimen {int(rv)} guardadas: {fig_path_c}")

# Boxplots por régimen
...
for ax, var in zip(ax_b, SYSTEMIC_VARS):
    data_box = [
        final[final["Regime_economic"] == rv][var].dropna().values
        for rv in sorted(final["Regime_economic"].dropna().unique())
    ]
    labels_box = [
        f"R{int(rv)}\n({REGIME_LABELS.get(int(rv), '')})"
        for rv in sorted(final["Regime_economic"].dropna().unique())
    ]
    bp = ax.boxplot(data_box, labels=labels_box, patch_artist=True, notch=True)
    colors_bp = [REGIME_COLORS.get(int(rv), "#aec7e8")
                 for rv in sorted(final["Regime_economic"].dropna().unique())]
...
plt.tight_layout()
fig_path_b = f"{FIGURES}/boxplot_regimes_{timestamp}.png"
plt.savefig(fig_path_b, dpi=150, bbox_inches="tight")
plt.show()
print(f"  ✔ Boxplots guardados: {fig_path_b}")

# ============================================================
# 7. ANÁLISIS ESTADÍSTICO: DIFERENCIAS ENTRE REGÍMENES
# ============================================================
print("\n" + "="*60)
...
for var in SYSTEMIC_VARS:
    ...
    if len(groups_valid) >= 2:
        # Kruskal-Wallis (no paramétrico, robusto)
        ...
        for rv, g in zip(regime_vals, groups):
            stat_results[-1][f"media_R{int(rv)}"] = round(g.mean(), 4) if len(g) else np.nan
...
print(f"  ✔ Tests guardados: {stats_path}")
...
# ─ 8.1 Identificar variable dependiente ─────────────────────
CANDIDATE_DEPVARS = ["Fragility", "ISI", "ISI_extended", "FAI"]
dep_var = next((v for v in CANDIDATE_DEPVARS if v in final.columns), None)

if dep_var is None:
    print("  ⚠ No se identificó variable dependiente canónica. "
          "Usando primera variable sistémica disponible.")
    dep_var = SYSTEMIC_VARS[0]

indep_vars = [v for v in SYSTEMIC_VARS if v != dep_var]
print(f"  Variable dependiente : {dep_var}")
print(f"  Variables independientes : {indep_vars}")

# ─ 8.2 OLS Pooled por régimen ────────────────────────────────
ols_results = {}

for rv in regime_vals:
    sub = final[final["Regime_economic"] == rv].copy()
    sub_clean = sub[[dep_var] + indep_vars + REGIME_DUMMY_COLS].dropna()

    if len(sub_clean) < max(10, len(indep_vars) + 2):
        print(f"  ⚠ Régimen {int(rv)}: insuficientes observaciones ({len(sub_clean)}). "
              "Se omite.")
        continue
    ...
    model = OLS(y, X).fit(cov_type="HC3")   # errores robustos heterocedásticos
    ols_results[int(rv)] = model

    print(f"\n── OLS Régimen {int(rv)} "
          f"({REGIME_LABELS.get(int(rv), '')}) ──")
    print(f"   N={model.nobs:.0f}  R²={model.rsquared:.4f}  "
          f"R²adj={model.rsquared_adj:.4f}  "
          f"F-stat={model.fvalue:.2f}(p={model.f_pvalue:.4f})")
    ...
    print(f"   ✔ Coeficientes guardados: {coef_path}")

# ─ 8.3 Modelo panel FE (si linearmodels está disponible) ────
if HAS_LINEARMODELS and entity_col and len(indep_vars) > 0:
    print("\n── Panel OLS — Efectos Fijos ──")
    try:
        panel_data = final.set_index([entity_col, "date"])
        panel_data = panel_data[[dep_var] + indep_vars + REGIME_DUMMY_COLS].dropna()

        y_pan  = panel_data[dep_var]
        X_pan  = add_constant(panel_data[indep_vars + REGIME_DUMMY_COLS[1:]])
        ...
        print(f"  ✔ Panel FE guardado: {fe_path}")

    except Exception as e:
        print(f"  ⚠ Panel FE falló: {e}")

# ============================================================
# 9. ANÁLISIS DE REGÍMENES: DURACIÓN Y TRANSICIONES
# ============================================================
print("\n" + "="*60)
print("9. ANÁLISIS DE DURACIÓN Y MATRIZ DE TRANSICIÓN")
print("="*60)

# ─ 9.1 Duración de episodios ─────────────────────────────────
def compute_durations(series: pd.Series) -> pd.DataFrame:
    """Calcula la duración de cada episodio de régimen."""
    records = []
    current = None
    start   = None
...
print("Duración promedio por régimen:")
...
# ─ 9.2 Matriz de transición empírica ────────────────────────
...
print(f"  ✔ Matriz de transición guardada: {trans_path}")

# Heatmap de transición
fig_t, ax_t = plt.subplots(figsize=(6, 5))
sns.heatmap(
    trans_df.astype(float),
    annot=True, fmt=".2f", cmap="Blues",
    linewidths=.5, ax=ax_t, vmin=0, vmax=1
)
...
plt.show()
print(f"  ✔ Heatmap guardado: {fig_path_t}")

# ============================================================
# 10. GUARDAR DATASET FINAL Y CHECKPOINT
# ============================================================
print("\n" + "="*60)
...
# Dataset completo
final_path = f"{BASE}/final_dataset_{timestamp}.csv"
final.to_csv(final_path, index=False)
print(f"  ✔ Dataset final: {final_path}  ({final.shape})")

# Parquet (más eficiente para cargas futuras)
try:
    pq_path = f"{BASE}/final_dataset_{timestamp}.parquet"
    final.to_parquet(pq_path, index=False)
    print(f"  ✔ Parquet guardado: {pq_path}")
except Exception as e:
    print(f"  ⚠ Parquet no disponible: {e}")

# Checkpoint de metadata
checkpoint_meta = {
    "timestamp"        : timestamp,
    ...
    "figures"          : [fig_path, fig_path_b, fig_path_t],
...
print(f"  ✔ Checkpoint JSON: {ck_path}")

# Copiar el propio script al checkpoint (reproducibilidad)
try:
    script_src = __file__
    shutil.copy(script_src, f"{CHECKPOINT}/script_{timestamp}.py")
except Exception:
    pass  # en Colab __file__ no existe; no crítico
...
print(f"  Observaciones      : {len(final):,}")
if entity_col:
...
print(f"\nModelos estimados:")
for rv, m in ols_results.items():
    print(f"  OLS Régimen {rv} ({REGIME_LABELS.get(rv,'')}) "
